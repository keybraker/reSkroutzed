...
export function addBlockedIndication(visible: boolean, language: Language, sponsoredCount: number): void {
  const existingBlockIndicationButton = document.getElementById(
    "sponsored-flagger-button"
  ) as HTMLButtonElement | null;

  if (existingBlockIndicationButton) {
    updateDisplayText(existingBlockIndicationButton, false, language, visible, sponsoredCount);
    return;
  }

  addButtonToFilters(language, visible, sponsoredCount);
  addButtonToOrderOptions(language, visible, sponsoredCount);
}
...
function updateDisplayText(
  ...
  sponsoredButtonToggle.innerHTML = `${icon} ${text}: ${sponsoredCount}`;
}

function createSponsoredIndicatorButton(isProduct: boolean = true, language: Language, visible: boolean, sponsoredCount: number): HTMLButtonElement {
  ...
  updateDisplayText(sponsoredButtonToggle, isProduct, language, visible, sponsoredCount);
...
function addButtonToFilters(language: Language, visible: boolean, sponsoredCount: number, existingButton?: HTMLButtonElement): void {
  const liOfferElements = document.querySelector("li.sku-offers");
  if (liOfferElements) {
    insertElement(
      liOfferElements.parentNode as Node,
      existingButton ?? createSponsoredIndicatorButton(true, language, visible, sponsoredCount),
      liOfferElements
    );
  }
}

function addButtonToOrderOptions(language: Language, visible: boolean, sponsoredCount: number, existingButton?: HTMLButtonElement): void {
  const orderOptionsDiv = document.getElementById("order-options");
  if (orderOptionsDiv) {
    insertElement(
      orderOptionsDiv.parentNode as Node,
      existingButton ?? createSponsoredIndicatorButton(false, language, visible, sponsoredCount),
